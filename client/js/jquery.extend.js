...
$.extend({
	...
	getUrlParam: function (paramName) {
		...
		///	 取當前页面GET參數值
		/// </summary>
		/// <param name="paramName" type="string">參數名称</param>
...
$.fn.confirm = function (options) {
	...
	/// </param>
	if (!options) options = { };
	var html = $.operatorTemplate.replace("取消", options.cancelTitle || "取消").format('<div class="left confirm">&nbsp;</div><div class="text">{0}</div>'.format(options.text || "无提示"), 'icon', '<input id="btnWinOK" name="btnWinOK" class="btnWinOK" type="button" value="' + (options.okTitle || "确定") + '" /> ');
	if (!options.html) options.html = html;
	if (!options.align) options.align = "center center";
	this.window(options);
};
...
$.fn.notice = function (options) {
	...
	///	 &#10;	text:	   內容 為null時是隱藏
	///	 &#10;	width:	  寬
	///	 &#10;	type:	   默認状态 ok,warning,error,process
	///	 &#10;	interval:   自動关闭(秒)
	///	 &#10;	classEx:	附加樣式類
	///	 &#10;	styleEx:	附加樣式
	...
	var defaultOptions = {
		...
		text: null,			 //內容
		width: 0,			   //寬
		type: "ok",			 //默認状态 ok,warning,error,process
		interval: 0,			//自动关闭
		classEx: "",			//附加樣式類
		styleEx: "",			//附加樣式
