...
abstract class Captcha
{
	...
	public function update_response_session()
	{
		...
		//write_session('captcha_response', sha1(UTF8::strtoupper($this->response)));
	}
	...
	public static function valid($response)
	{
		...
		//$result = (bool) (sha1(UTF8::strtoupper($response)) === read_session('captcha_response'));
		...
		if ($counted !== TRUE)
		{
			...
			if ($result === TRUE)
			{
				...
				//Captcha::instance()->valid_count(read_session('captcha_valid_count') + 1);
			}
			// Invalid response
			else
			{
				Captcha::instance()->invalid_count(Session::instance()->get('captcha_invalid_count') + 1);
				//Captcha::instance()->invalid_count(read_session('captcha_invalid_count') + 1);
			}
		}
	...
	public function valid_count($new_count = NULL, $invalid = FALSE)
	{
		...
		if ($new_count !== NULL)
		{
			...
			else
			{
				...
				//write_session($session, (int) $new_count);
			}
		...
		//return (int) read_session($session);
	}
